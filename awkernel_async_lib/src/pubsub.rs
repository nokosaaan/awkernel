    ...
    T: Clone + Sync + Send,
{
    ...
    pub async fn send_with_period_index(
        ...
        record_publish_timestamp(index, start, pub_id, dag_id, node_id);
...
mod period_index_propagation_tests {
    ...
    fn send_with_period_index_propagates_period_index_to_receiver() {
        block_on(async {
            ...
            publisher.send_with_period_index(42, 1, 7, 5, 99).await;
    ...
    fn tuple_recv_all_with_period_index_returns_shared_period_index() {
        block_on(async {
            ...
            publisher1.send_with_period_index(10, 11, 3, 100, 21).await;
            publisher2.send_with_period_index(20, 12, 3, 100, 22).await;
    ...
    ($(($T:ident, $idx:tt, $idx2:tt)),+) => {
        ...
        impl<$($T: Clone + Sync + Send + 'static),+> MultipleSender for ($(Publisher<$T>,)+) {
            ...
            fn send_all_with_period_index(
                ...
                Box::pin(async move {
                    $(
                        $idx.send_with_period_index($idx2, pub_id, index, dag_id, node_id).await;
                    )+
                })
            }
        }
